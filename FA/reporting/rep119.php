...
function GetSalesOrders($from, $to)
{
	...
	$todate = date2sql($to);
        ...
        $sql= "SELECT ".TB_PREF."item_codes.stock_id idProdDist, 
            ".TB_PREF."item_codes.description nombre, 
            ".TB_PREF."item_codes.stock_id idProdNestle 
            FROM ".TB_PREF."item_codes
            WHERE ".TB_PREF."item_codes.inactive = 0
                AND ".TB_PREF."item_codes.category_id != 27";
...
function print_csv_files()
{
    ...
    $elCSV = '"idProdDist","nombre","idProdNestle"';
    $elCSV .= "\r\n";
        
    $csvName = "PR". str_replace('/','',$from) .".csv";
	...
	while ($myrow=db_fetch($result))
	{
            ...
            $elCSV .= '"'.$myrow['idProdDist'].'","'.$myrow['nombre'].'","'.
                      $myrow['idProdNestle'].'"'."\r\n";
