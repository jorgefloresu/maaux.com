...
function GetSalesOrders($from, $to)
{
	...
	$todate = date2sql($to);
        ...
        $sql= "SELECT ".TB_PREF."debtors_master.debtor_no idTiendaDist,
            ".TB_PREF."debtors_master.name nombre,
            ".TB_PREF."debtors_master.address direccion,
            ".TB_PREF."cust_branch.contact_name propietario,
            '' telefonos, 
            '' idTiendaNestle
            FROM ".TB_PREF."cust_branch
            INNER JOIN ".TB_PREF."debtors_master 
            ON ".TB_PREF."cust_branch.debtor_no = ".TB_PREF."debtors_master.debtor_no";
...
function print_csv_files()
{
    ...
    $elCSV = '"idTiendaDist","nombre","direccion","propietario","telefonos","idTiendaNestle"';
    $elCSV .= "\r\n";
        
    $csvName = "TD". str_replace('/','',$from) .".csv";
	...
	while ($myrow=db_fetch($result))
	{
            ...
            $elCSV .= '"'.$myrow['idTiendaDist'].'","'.$myrow['nombre'].'","'.
                      $myrow['direccion'].'","'.$myrow['propietario'].'","'.
                      $myrow['telefonos'].'","'.$myrow['idTiendaNestle'].'"'."\r\n";
